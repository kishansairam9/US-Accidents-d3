...
// Monthly - line chart
// Takes input array of data and plots Line chart with respect to month vs number of accidents
...
d3.csv("../../cleaned.csv").then(function (tot) {
  ...
  // d3 can't handle whole 1M data in 2019 year. Hence using 1/5 of lenght which browser can handle
  inp = _.sample(tot, tot.length / 5);
  ...
  // Show sample Table of data
  {
    ...
    tr.selectAll("td")
      .data(d => {
        return [d.ID, d.Source, d.Severity, d.Time, d.Lat, d.Lng, d.Description, d.State, d.Weather, d.Object]
      })
      .enter()
      .append("td")
      .text(function (d) {
...
const createPlots = (out) => {
  ...
  // Takes input array of data and plots Line chart with respect to weekday vs number of accidents
  const plotWeekly = (out) => {
    ...
    svg.append("text")
      ...
      .attr("font-size", "10px")
      .text(`Accidents`)
      .attr("transform", `rotate(-90 -25, ${height / 2 + 10})`)
    ...
    svg.append("g")
      .attr("class", "y weekly-axis weekly-left-axis")
      .call(d3.axisLeft(y))
      .append("text")
  ...
  // Weather - pie chart
  // Takes input array of data and plots Pie chart of distribution of accidents with respect to weather condition
  ...
  // Takes input array of data and plots Bar chart with respect to time of day vs number of accidents
  const plotTime = (out) => {
  ...
  // Map 
  // Takes input array of data and plots the main interactive map
  ...
  // Toggle between year plot and current month
  const togglePlots = () => {
    if (year) {
  ...
  // Updates all plots with month number given
  ...
  // Initially plot January
  updatePlots(0)

  // Create slider for months
  ...
  // Add event listener for toggling of year / month analysis plots
